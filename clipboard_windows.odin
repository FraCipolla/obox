...
foreign import lib_c "system:libcmt"
@(default_calling_convention = "c")
foreign lib_c {
    _popen  :: proc(command: cstring, mode: cstring) -> ^libc.FILE ---
    _pclose :: proc(stream: ^libc.FILE) -> i32 ---
}

copy_to_os_clipboard :: proc(text: string) {
    pipe_to_cli("clip.exe", text)
}

read_os_clipboard :: proc(allocator := context.temp_allocator) -> string {
    return read_from_cli("powershell -NoProfile -Command Get-Clipboard", allocator)
}

pipe_to_cli :: proc(cmd: string, text: string) -> bool {
    cmd_cstring := strings.clone_to_cstring(cmd, context.temp_allocator)
    pipe := _popen(cmd_cstring, "w")
